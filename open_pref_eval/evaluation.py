...
def extract_logps(model, batch, step: int, score_fn: Callable=score_weighted, include_raw=False):
    ...
    adj_cho = torch.logsumexp(2 * chosen_t_logps, dim=-1, keepdim=True)  # same as sum(probs**2) in log space
    chosen_weights = ((chosen_t_logps - adj_cho ) * chosen_mask).sum(-1) / chosen_mask.sum(-1)

    adj_rej = torch.logsumexp(2 * rejected_t_logps, dim=-1, keepdim=True)  # same as sum(probs**2) in log space
    rejected_weights =(( rejected_t_logps- adj_rej) * rejected_mask).sum(-1) / rejected_mask.sum(-1)
...
@torch.no_grad()
def eval_dataset(model: PreTrainedModel, tokenizer: PreTrainedTokenizerBase, dataset: Union[Dataset,str], adapter_names:Optional[List[str]]= None,  max_prompt_length:int=512, max_length:Optional[int]=1024, batch_size=2, verbose=1, **kwargs) -> pd.DataFrame:
    """
    ...
    eval_dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=data_collator, num_workers=0, shuffle=False, drop_last=False)
...
def eval_datasets(model, tokenizer, datasets: List[Dataset], verbose=1, **kwargs) -> pd.DataFrame:
    dfs = []
...
def evaluate_model(model:PreTrainedModel, tokenizer:PreTrainedTokenizerBase, datasets: List[Dataset], score_fn=score_weighted, verbose=1, **kwargs) -> pd.DataFrame:
    
    df_raw = eval_datasets(model, tokenizer, datasets, score_fn=score_fn, verbose=verbose, **kwargs)
...
def evaluate_models(datasets: List[Dataset], model_names: List[str], model_kwargs={}, **kwargs):
    dfs_raw = []
